...
//
// A pruned entry is never redownloaded automatically: nothing in this package deletes
// it based on the channel's CURRENT retention/cutoff window (that was an actual bug --
// see PurgeExpiredVideos and the reasoning below). It is eventually eligible for hard
// deletion, but only via a boundary that can't be silently invalidated by an ordinary
// later settings change: PurgeExpiredVideos's permanentBookkeepingGraceDays.
//
// retention_days and cutoff_date are user-editable at any time, and the RSS discovery
// window (BuildChannelSinceTime) is derived from whatever they're currently set to. If a
// pruned video's bookkeeping were trimmed the moment it fell outside TODAY's window,
// then that window widened later (e.g. the user raises retention), the video could
// resurface via RSS and genuinely get re-downloaded -- exactly what "pruned" promises
// will never happen. So eviction is instead gated on either the channel's cutoff_date
// (a genuine floor on the window regardless of retention_days -- see
// BuildChannelSinceTime) or a large fixed grace period, neither of which moves just
// because retention_days was edited. Removing the channel entirely (RemoveChannel) is
// the only way to forget a pruned video immediately.
...
// DismissAllFeedVideos moves every pending video in a channel's feed to 'pruned' in one
// batch. Matches the per-video dismiss semantics (AddPrunedVideo + RemoveFeedVideo
// together end at status='pruned') but applied to the whole pending list at once, so
// none of them are re-downloaded or re-surfaced by future feed scans.
func (s *Storage) DismissAllFeedVideos(channelID string) (int64, error) {
	res, err := s.db.Exec(`UPDATE videos SET status = 'pruned'
