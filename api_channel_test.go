...
// TestConvertToChannelPreservesNotYetDownloadedVideo is a regression test: a video
// still pending (no DownloadedVideos entry yet) included in a convert-to-channel batch
// must not simply vanish. Before this fix, only DownloadedVideos were carried over to
// the new channel, so a not-yet-downloaded video's RemoveVideo call at the end of the
// handler deleted its only tracking record with nothing added in its place.
func TestConvertToChannelPreservesNotYetDownloadedVideo(t *testing.T) {
	tmpDir := t.TempDir()
	store, err := storage.NewStorage(filepath.Join(tmpDir, "data.db"))
	if err != nil {
		t.Fatalf("NewStorage() error = %v", err)
	...
	if err := store.AddVideo(storage.Video{
		ID: "vid-downloaded", Title: "Downloaded", Uploader: "Test Creator", UploaderID: "UCtest123",
		DownloadedVideos: []storage.DownloadedVideo{{ID: "vid-downloaded", Title: "Downloaded"}},
	}); err != nil {
		t.Fatalf("AddVideo() error = %v", err)
	}
	if err := store.AddVideo(storage.Video{
		ID: "vid-pending", Title: "Pending", URL: "https://youtu.be/vid-pending",
		Uploader: "Test Creator", UploaderID: "UCtest123",
	}); err != nil {
		t.Fatalf("AddVideo() error = %v", err)
	}
	...
	body, _ := json.Marshal(map[string]interface{}{
		"uploader_name": "Test Creator",
		"uploader_id":   "UCtest123",
		"video_ids":     []string{"vid-downloaded", "vid-pending"},
	})
	req := httptest.NewRequest(http.MethodPost, "/api/videos/convert-to-channel", bytes.NewReader(body))
	rec := httptest.NewRecorder()
	api.handleConvertToChannel(rec, req)
	...
	ch := channels[0]
	if len(ch.DownloadedVideos) != 1 || ch.DownloadedVideos[0].ID != "vid-downloaded" {
		t.Errorf("expected vid-downloaded present, got %+v", ch.DownloadedVideos)
	}
	if len(ch.FeedVideos) != 1 || ch.FeedVideos[0].ID != "vid-pending" {
		t.Fatalf("expected vid-pending to survive as a pending FeedVideo entry, got %+v", ch.FeedVideos)
	}
	if ch.FeedVideos[0].URL != "https://youtu.be/vid-pending" {
		t.Errorf("expected pending video's URL preserved, got %q", ch.FeedVideos[0].URL)
	}

	if videos := store.GetVideos(); len(videos) != 0 {
		t.Errorf("expected 0 individual video entries remaining, got %d: %+v", len(videos), videos)
	}
}

// TestConvertToChannelMergesMultipleVideosIncludingPending exercises merging 2+
// individual videos into an ALREADY-EXISTING channel in one request -- the prior test
// suite only covered a single video merging into an existing channel, and separately
// covered 2+ videos only for the "create a brand new channel" path. It also covers a
// still-pending video in the merge-into-existing branch specifically, since that
// branch's fix (UpsertFeedVideo) is separate code from the create-new branch's fix.
func TestConvertToChannelMergesMultipleVideosIncludingPending(t *testing.T) {
	tmpDir := t.TempDir()
	store, err := storage.NewStorage(filepath.Join(tmpDir, "data.db"))
	if err != nil {
		t.Fatalf("NewStorage() error = %v", err)
	...
	existingChannel := storage.Channel{
		ID:               "UCmergemulti",
		Name:             "Merge Multi Creator",
		URL:              "https://www.youtube.com/channel/UCmergemulti",
	...
	if err := store.AddVideo(storage.Video{
		ID: "solo-vid-1", Title: "Solo One", Uploader: "Merge Multi Creator", UploaderID: "UCmergemulti",
		DownloadedVideos: []storage.DownloadedVideo{{ID: "solo-vid-1", Title: "New Download 1"}},
	}); err != nil {
		t.Fatalf("AddVideo() error = %v", err)
	}
	if err := store.AddVideo(storage.Video{
		ID: "solo-vid-2", Title: "Solo Two", Uploader: "Merge Multi Creator", UploaderID: "UCmergemulti",
		DownloadedVideos: []storage.DownloadedVideo{{ID: "solo-vid-2", Title: "New Download 2"}},
	}); err != nil {
		t.Fatalf("AddVideo() error = %v", err)
	}
	if err := store.AddVideo(storage.Video{
		ID: "solo-vid-pending", Title: "Solo Pending", URL: "https://youtu.be/solo-vid-pending",
		Uploader: "Merge Multi Creator", UploaderID: "UCmergemulti",
	}); err != nil {
		t.Fatalf("AddVideo() error = %v", err)
	}
	...
	body, _ := json.Marshal(map[string]interface{}{
		"uploader_name": "Merge Multi Creator",
		"uploader_id":   "UCmergemulti",
		"video_ids":     []string{"solo-vid-1", "solo-vid-2", "solo-vid-pending"},
	})
	req := httptest.NewRequest(http.MethodPost, "/api/videos/convert-to-channel", bytes.NewReader(body))
	rec := httptest.NewRecorder()
	api.handleConvertToChannel(rec, req)
	...
	channels := store.GetChannels()
	if len(channels) != 1 {
		t.Fatalf("expected exactly 1 channel (no duplicate created), got %d", len(channels))
	}
	ch := channels[0]
	if len(ch.DownloadedVideos) != 3 {
		t.Errorf("expected 3 downloaded videos (existing-dv + 2 merged), got %d: %+v", len(ch.DownloadedVideos), ch.DownloadedVideos)
	}
	if len(ch.FeedVideos) != 1 || ch.FeedVideos[0].ID != "solo-vid-pending" {
		t.Fatalf("expected solo-vid-pending to survive as a pending FeedVideo entry, got %+v", ch.FeedVideos)
	}
