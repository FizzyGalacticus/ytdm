...
func checkAndQueueChannel(ctx context.Context, channelID string, store *storage.Storage, downloader *Downloader, downloadQueue chan<- DownloadRequest) bool {
	...
	if since.IsZero() {
		...
		// Pending feed entries are safe to hard-prune here: losing one just means it may
		// get freshly re-evaluated on a later scan, which is the intended behavior, not
		// a broken guarantee. Pruned/downloaded bookkeeping is deliberately NOT trimmed
		// this way (or at all, automatically) -- see storage.AddPrunedVideo and
		// PurgeExpiredVideos. `since` is derived from the channel's CURRENT retention
		// setting and is only a valid "will never resurface" boundary for as long as
		// that setting never changes; if a user later widens retention (or removes a
		// cutoff), a video whose pruned-memory was trimmed under the old, narrower
		// window could resurface via RSS and actually be re-downloaded. Since retention
		// is user-editable at any time, pruned/downloaded rows must remain permanent.
...
func RunScheduler(ctx context.Context, config *Config, store *storage.Storage) {
	...
	// Backstop hard-delete of video rows that are more than 2x past their retention
	// window (see storage.PurgeExpiredVideos). Runs once shortly after startup, then at
	// most once every 24h -- this is a safety net for rows normal cleanup missed, not
	// part of the regular per-channel scan cycle, so it doesn't need to run often.
	var lastPurge time.Time
	purgeExpiredVideos := func() {
		...
		config.RUnlock()
		deleted, err := store.PurgeExpiredVideos(time.Now(), defaultRetention)
		if err != nil {
			log.Printf("Error purging expired video rows: %v", err)
			return
		}
		if deleted > 0 {
			log.Printf("Purged %d video row(s) more than 2x past their retention window", deleted)
		}
	}
